...
Connect and confirm the channel, then select **Start Uploads**. Review video count, total size, dates, thumbnail count, duplicates and limitations. Confirm explicitly. Uploads run sequentially with resumable chunks and retry transient failures. The computer stays awake, but UPLOAD PLUGG never shuts it down or restarts it.
